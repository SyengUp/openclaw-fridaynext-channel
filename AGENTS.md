...
3. **`src/http/server.ts`** + **`src/http/handlers/*`** — `/friday-next/*` routes. Full route table:
   ...
   - `POST /friday-next/sessions/bind` — attach a device to a session's live stream (`src/http/handlers/sessions-bind.ts`): registers the device in the watched-session map (`bindFridayDeviceToSession`, durable in `~/.openclaw/friday-next/session-binds.json`) and injects the session's buffered `agent` frames (`src/sse/session-replay-buffer.ts`, per-session ring ≤ 400 frames / 60 sessions LRU) into the device's durable SSE queue. From then on `forwardAgentEventRaw` forwards that session's `agent` frames to every bound device — the mechanism by which a conversation started in Control UI / WebChat streams into the iOS app even though the app never POSTed to that session. Additive: a POST-registered owner keeps the owner path; bound devices receive via an extra direct broadcast.
