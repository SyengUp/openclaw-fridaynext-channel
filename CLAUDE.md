...
3. **`src/http/server.ts`** + **`src/http/handlers/*`** — `/friday-next/*` routes. Full route table:
   ...
   - `GET /friday-next/agents` — list configured agents (`handleAgentsList`); returns `{ ok, agents, defaultAgentId }` where each agent has `id`/`name`/`description`/`model`/`thinkingDefault`/`isDefault`/`emoji`/`avatar` (+ `greeting` when that agent has a custom home-greeting override). Reads `cfg.agents.list` directly from the runtime config (same pattern as models-list, not via gateway dispatch); falls back to a single implicit `main` agent when none are configured.
   ...
   - `GET|PUT /friday-next/agents/{id}/greeting` — a single agent's home-greeting override (首页问候语, `handleAgentGreeting`). GET → `{ greeting: string | null }`; PUT `{ greeting }` (trimmed, ≤60 chars, empty clears the override). Stored by `src/agent-greetings/greetings-store.ts` at `~/.openclaw/friday-next/agent-greetings/greetings.json` — deliberately NOT in `agents.list[]` (core `AgentEntrySchema` is strict; an unknown field would fail config validation). No override → app falls back to its localized default.
...
20. **`src/public-access/frpc-manager.ts`** — FridayTunnel public access: now a thin host adapter over `@syengup/tunnel-edge`'s `startTunnelRuntime`. The shared `TunnelRuntime` owns the frpc + edge + standby lifecycle (frpc download/checksum/spawn/pidfile/respawn, edge in-process/external mode, control-plane standby polling, tunnel watchdog, cert renewal) under `~/.openclaw/friday-next/public-access/`. The plugin supplies the OpenClaw-specific host adapters: `buildTunnelBackends(cfg)` for the routing table, the live App Attest gate, the external-edge attest verifier URL, the pairing-voucher port, and the scoped `[friday-next:public-access]` logger. `startPublicAccess`/`stopPublicAccess`/`reconcileServedSubdomains`/`getPairingInfo`/`currentServedSubdomains` delegate to the singleton runtime; `resolveConductorPort`/`resolveConductorHost`/`resolveEdgeMode`/`edgeConfigFor` remain pure config helpers. Pure helpers (`isValidSubdomainLabel`, `normalizedServedSubdomains`, pidfile guards, process-list parsing, `parseControlPlaneBackends`, `backendTablesEqual`, `TunnelHealthTracker`, `isTunnelHealthyStatus`) are re-exported from `@syengup/tunnel-edge`; `expectedFrpcArchiveSHA256`/`frpcDownloadSources` are re-exported from the local `frpc-download-source.ts`. The edge routing table is `buildTunnelBackends(cfg)`: `openclaw` (`/friday-next`, `/friday-next-admin`, `/gateway`, `/__openclaw__`, with the historical deny + attest-exempt paths) plus `conductor` (`/cap` with the CAP allowlist) when `resolveConductorPort(cfg) > 0`; the runtime treats the control plane's `backends` response as authoritative when valid and otherwise falls back to the locally built table. Local `standby-loop-guard.ts`/`tunnel-watchdog-policy.ts` remain only for legacy unit tests (production uses the edge package runtime).
...
- **`src/agent-greetings/greetings-store.ts`** + **`src/http/handlers/agent-greetings.ts`** — `GET|PUT …/greeting`. Per-agent home-greeting override stored in the plugin's own JSON (gateway-side sync), because the core `agents.list[]` schema is strict. Empty PUT clears the override; `agents-list` merges `greeting` per agent so the app's home page reads it from `GET /agents`.
