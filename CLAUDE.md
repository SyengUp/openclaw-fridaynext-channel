...
3. **`src/http/server.ts`** + **`src/http/handlers/*`** — `/friday-next/*` routes. Full route table:
   ...
   - `POST /friday-next/sessions/bind` — attaches a device to an existing session. Runs started by Control UI, WebChat, or core recovery are adopted as non-dispatchable `observed` v3 runs; buffered frames and future live frames remain replayable across gateway restarts, and each bound device gets its own delivery ledger.
