...
3. **`src/http/server.ts`** + **`src/http/handlers/*`** — `/friday-next/*` routes. Full route table:
   - `POST /friday-next/v3/messages` · `GET /friday-next/v3/events` · `POST /friday-next/v3/ack` — durable multi-session command/stream protocol. `DurableRunStore` journals semantic run events into per-device delivery ledgers with independent event ids and acknowledgements.
   - `POST /friday-next/sessions/bind` — attaches a device to an existing session. Unfinished runs started by Control UI, WebChat, or core recovery are adopted as non-dispatchable `observed` v3 runs; completed buffered runs stay on the transcript/history path instead of being re-presented as live output, and each bound device gets its own delivery ledger for future live frames.
