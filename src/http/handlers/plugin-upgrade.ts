 ...
 *
 * On success the handler schedules a managed-service gateway restart (2s after the
...
/** The managed-service restart may terminate this process before the command
 *  returns, so keep the timeout bounded without depending on its response. */
 ...
 *
 * Use the plain managed-service restart path here instead of `--safe`: safe
 * restart still drains pending replies after its active-work deferral gate, and
 * that close-stage drain can keep the app waiting for a reconnect. The install
 * and HTTP response are complete before this detached callback runs.
 */
async function requestGatewayRestart(
  ...
  const result = await runRestart(["openclaw", "gateway", "restart"]);
...
async function runUpgradeInBackground(
  ...
  // the restart drops the connection, then trigger the managed-service restart.
  setTimeout(() => {
