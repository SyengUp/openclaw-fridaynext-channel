...
function setup() {
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "friday-push-"));
  roots.push(root);
  const store = new DurableRunStore(root);
  const send = vi.fn(async () => 202);
  const cancel = vi.fn(async () => 200);
...
describe("持久推送队列", () => {
  it("无 SSE 监听者时等待一秒，然后发送且重启不重复", async () => {
    ...
    const restarted = new PushRuntime(file, store, send, Date.now, cancel);
  ...
  it("前台确认可以早于推送扫描，且请求撤销会同步中继", async () => {
    ...
    push.handled("PHONE", id);
    vi.advanceTimersByTime(1000);
    await push.tick();
    expect(send).not.toHaveBeenCalled();
    expect(cancel).toHaveBeenCalledWith("grant", id);
  });
  it("失败后重启重试；用户取消抢先抑制 completion", async () => {
    const { store, send, cancel, file, push, run } = setup();
    send.mockResolvedValueOnce(503);
    store.appendRunEvent(run.runId, "run.completed", {});
    vi.advanceTimersByTime(1000);
    await push.tick();
    const restart = new PushRuntime(file, store, send, Date.now, cancel);
    await restart.tick();
    expect(send).toHaveBeenCalledTimes(1);
    restart.cancel(run.runId);
  ...
  it("询问终态先到不会补发旧询问；审批按真实到期时间截断", async () => {
    ...
    store.appendRunEvent(run.runId, "deliver.tool", {
      _sourceEventType: "deliver",
      _sourceEventData: { payload: { channelData: { askUser: { questionId: "q" } } } },
    });
    const wire = store.appendRunEvent(run.runId, "approval.request", {
      approvalId: "a",
  ...
  it("子会话和定时任务不产生执行结束通知，失败共用完成身份", () => {
    ...
    expect(pushCandidates({ ...wire, sourceKind: "cron" })).toEqual([]);
    expect(pushCandidates({ ...wire, sourceKind: "heartbeat" })).toEqual([]);
    expect(pushCandidates({ ...wire, sourceKind: undefined })).toEqual([]);
