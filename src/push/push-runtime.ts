...
type State = {
  ...
  cancellations: Record<
    string,
    { deviceId: string; id: string; expiresAt: number; nextAt: number }
  >;
};
    ...
    crypto
      ...
      .update(
        JSON.stringify([
          ...
          event.kind === "failed" ? "completed" : event.kind,
...
export function pushCandidates(
  wire: DurableRuntimeEvent,
): Array<{ event: PushEvent; terminal: boolean }> {
  const common = {
  ...
  if (["run.completed", "run.failed"].includes(wire.eventType)) {
    if (wire.sourceKind !== "session" || wire.parentRunId) return [];
    return [
      {
        event: {
          ...common,
          kind: wire.eventType === "run.failed" ? "failed" : "completed",
          expiresAt: wire.occurredAt + 86400000,
        },
        terminal: false,
      },
    ];
  ...
  const sources = Array.isArray(p._sourceEventBatch)
    ? p._sourceEventBatch.map(obj)
    : [{ type: p._sourceEventType, data: p._sourceEventData }];
  if (!p._sourceEventType && !Array.isArray(p._sourceEventBatch)) {
    for (const type of ["approval", "question"])
      if (wire.eventType.startsWith(type + "."))
        sources.push({ type, data: { ...p, op: wire.eventType.slice(type.length + 1) } });
  }
  ...
  for (const source of sources) {
    ...
    const expiry =
      typeof data.expiresAtMs === "number" ? data.expiresAtMs : wire.occurredAt + 3600000;
    result.push({
      event: { ...common, kind, requestId, expiresAt: Math.min(expiry, wire.occurredAt + 3600000) },
      terminal,
    });
  }
  return result;
...
export class PushRuntime {
  ...
  constructor(
    private file: string,
    private store: DurableRunStore,
    private post: (grant: string, event: PushEvent) => Promise<number> = async (
      pushGrant,
      event,
    ) => {
      const response = await fetch(PUSH_ORIGIN + "/v1/push/events", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ pushGrant, event }),
        signal: AbortSignal.timeout(10000),
      });
      return response.status;
    },
    private now = Date.now,
    private cancelPost: (grant: string, id: string) => Promise<number> = async (
      pushGrant,
      notificationId,
    ) => {
      const response = await fetch(PUSH_ORIGIN + "/v1/push/cancel", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ pushGrant, notificationId }),
        signal: AbortSignal.timeout(10000),
      });
      return response.status;
    },
    ...
    this.state = fs.existsSync(file)
      ? (JSON.parse(fs.readFileSync(file, "utf8")) as State)
      : { registrations: {}, jobs: {}, handled: {}, cancelled: {}, cancellations: {} };
    this.state.cancellations ??= {};
    for (const r of Object.values(this.state.registrations))
      store.setPushCursor(r.deviceId, r.cursor);
  }
  private save(): void {
  ...
  register(input: Omit<Registration, "cursor">): void {
    ...
    this.state.registrations[input.deviceId] = { ...input, cursor };
    this.save();
  }
  remove(deviceId: string): void {
    delete this.state.registrations[deviceId];
    for (const [key, job] of Object.entries(this.state.jobs))
      if (job.deviceId === deviceId) delete this.state.jobs[key];
    this.save();
    this.store.setPushCursor(deviceId, null);
  }
  handled(deviceId: string, id: string): void {
    const key = deviceId + ":" + id;
    this.state.handled[key] = this.now() + 86400000;
    delete this.state.jobs[key];
    if (this.state.registrations[deviceId])
      this.state.cancellations[key] = {
        deviceId,
        id,
  ...
  cancel(runId: string): void {
    ...
    for (const r of Object.values(this.state.registrations)) {
      for (const wire of this.store.eventsForRun(runId)) {
        ...
        for (const { event } of pushCandidates(wire))
          this.handled(r.deviceId, pushIdentity(r.profileId, event));
      }
    }
    this.save();
  }
  async tick(): Promise<void> {
    ...
    try {
      ...
      for (const r of Object.values(this.state.registrations)) {
        ...
        for (const wire of this.store.eventsAfter(r.deviceId, r.cursor, 1000)) {
          for (const candidate of pushCandidates(wire)) {
            const id = pushIdentity(r.profileId, candidate.event);
            const key = r.deviceId + ":" + id;
            if (candidate.terminal) this.handled(r.deviceId, id);
            else if (!this.state.handled[key] && !this.state.jobs[key])
              this.state.jobs[key] = {
                deviceId: r.deviceId,
                event: candidate.event,
                id,
                nextAt: wire.occurredAt + 1000,
                attempts: 0,
              };
          }
          r.cursor = wire.eventId;
          changed = true;
        }
        if (changed) {
          this.save();
          this.store.setPushCursor(r.deviceId, r.cursor);
        }
      }
      for (const [key, item] of Object.entries(this.state.cancellations)) {
        const r = this.state.registrations[item.deviceId];
        if (!r || item.expiresAt <= this.now()) {
          delete this.state.cancellations[key];
          changed = true;
          continue;
        }
        ...
        let status = 503;
        try {
          status = await this.cancelPost(r.pushGrant, item.id);
        } catch {
          /* 下次重试 */
        }
      ...
      for (const [key, job] of Object.entries(this.state.jobs)) {
        ...
        if (
          !r ||
          this.state.handled[key] ||
          this.state.cancelled[job.event.runId] ||
          !this.store.run(job.event.runId) ||
          job.event.expiresAt <= this.now()
        ) {
          delete this.state.jobs[key];
          changed = true;
          continue;
        ...
        let status: number;
        try {
          status = await this.post(r.pushGrant, job.event);
        } catch {
          status = 503;
        }
        if (status >= 200 && status < 300) {
          delete this.state.jobs[key];
          this.state.handled[key] = job.event.expiresAt;
        } else if (status === 403) this.remove(job.deviceId);
        else if (status === 429 || status >= 500) {
          job.attempts++;
          job.nextAt = this.now() + Math.min(300000, 1000 * 2 ** Math.min(job.attempts, 9));
        } else {
          delete this.state.jobs[key];
          console.error("[push] invalid_event", status);
        }
        changed = true;
        this.save();
      }
      for (const map of [this.state.handled, this.state.cancelled])
...
export function getPushRuntime(): PushRuntime {
  ...
  if (!active || active.root !== root)
    active = {
      root,
      value: new PushRuntime(path.join(root, "push-state.json"), getRuntimeV3Store()),
    };
  return active.value;
}
...
export function startPushRuntime(): void {
  ...
  timer = setInterval(() => {
    void getPushRuntime()
      .tick()
      .catch(() => console.error("[push] queue_failed"));
  }, 250);
  timer.unref();
}
