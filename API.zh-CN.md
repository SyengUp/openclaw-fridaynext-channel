...
- `GET /friday-next/prompt-capsules`
  ...
  `storeId` 首次访问时生成、之后永不变——客户端用它标记"同一个数据源"，所以同一网关走局域网 IP 还是公网中继域名都算一份；真换了网关则识别为新对端（只做并集、绝不删本地数据）。全新网关返回 `revision: 0` + 一个初始胶囊（编辑提示）。已有 store 文件即便列表为空也不再补种——空列表表示用户删过。
